...
NOTE: regardless of the conflict resolution method, all dependencies must be
satisfied in order for the transaction to succeed unless B<--nodeps> is used as
well.
